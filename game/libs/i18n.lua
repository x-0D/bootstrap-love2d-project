...
--- - Love2D integration
--- - Modding support: additive loading and merging

--- @class I18n
--- @field configure fun(options: I18nConfigureOptions): nil Configure the i18n library
--- @field load fun(dir: string?, clear: boolean?): nil Load translation files from a directory (additive by default)
...
--- @param target table Table to merge into
--- @param source table Table to merge from
...
--- Get nested value from table using dot or slash notation
--- @param t table<string, any> Table to search in
--- @param key string Dot or slash-separated key (e.g., "menu.items.play" or "menu/items/play")
...
--- Set nested value in table using dot or slash notation
...
--- @param dir string? Directory to load from (defaults to config.localesDir)
--- @param clear boolean? Whether to clear existing translations (default: false)
function i18n.load(dir, clear)
	 if clear then
		translations = {}
	 end

	 local targetDir = dir or config.localesDir

	 if not fileExists(targetDir) then
	 	 print("Warning: Translations directory '" .. targetDir .. "' not found")
	 	 return
	 end

	 local files = getFilesInDirectory(targetDir)
	 ...
	 for _, filename in ipairs(files) do
	 	 if filename:match("%.lua$") then
	 	 	 ...
	 	 	 if translationData then
				 ...
				 merge(translations[locale], translationData)
	 	 	 	 print("Loaded translations for locale: " .. locale .. " from " .. filepath)
	 	 	 end
	 	 end
	 end
end
...
--- Get fallback locale(s)
--- @return string|string[] locale Fallback locale identifier(s)
function i18n.getFallbackLocale()
	 return config.fallbackLocale
end

--- Set fallback locale(s)
--- @param locale string|string[] Locale identifier or array of identifiers
...
--- @param locale string Locale identifier
function i18n.addFallbackLocale(locale)
	 if type(config.fallbackLocale) ~= "table" then
		if config.fallbackLocale then
			config.fallbackLocale = { config.fallbackLocale }
		else
			config.fallbackLocale = {}
		end
	 end
	 table.insert(config.fallbackLocale, locale)
end

--- Remove a fallback locale
--- @param locale string Locale identifier
function i18n.removeFallbackLocale(locale)
	 if type(config.fallbackLocale) == "table" then
		for i, l in ipairs(config.fallbackLocale) do
			if l == locale then
				table.remove(config.fallbackLocale, i)
				break
			end
		end
	 elseif config.fallbackLocale == locale then
		config.fallbackLocale = nil
	 end
end

--- Translate a key
--- @param key string Translation key (supports dot and slash notation)
--- @param params table<string, any>? Parameters for interpolation
--- @param locale string? Locale override (uses current locale if not specified)
--- @param default string? Default value if translation not found
--- @return string translation Translated string or key/default if translation not found
function i18n.t(key, params, locale, default)
	 local targetLocale = locale or config.currentLocale

	 -- Build search order
	 local searchOrder = { targetLocale }

	 -- Add fallback locales
	 if type(config.fallbackLocale) == "table" then
	 	 for _, l in ipairs(config.fallbackLocale) do
	 	 	 table.insert(searchOrder, l)
	 	 end
	 elseif config.fallbackLocale then
	 	 table.insert(searchOrder, config.fallbackLocale)
	 end

	 -- Try to get translation in order
	 local translation = nil
	 local tried = {}
	 for _, l in ipairs(searchOrder) do
	 	 if not tried[l] then
	 	 	 tried[l] = true
	 	 	 if translations[l] then
	 	 	 	 translation = getNestedValue(translations[l], key)
	 	 	 	 if translation ~= nil then break end
	 	 	 end
	 	 end
	 end
